/* =========================================================================
   FOS Dashboard – app.js  ·  V2
   Vanilla JS · IndexedDB · lokal · offline-fähig
   Entitäten: subjects, tasks, exams, grades, appointments,
              internshipSites, internship (Tage), images
   V2-Kern: Daten über IDs verbunden (Klausur→Note→Fach→Schnitt),
            Praktikum als Hierarchie Stelle → Tag → Bericht/Bilder.
   ========================================================================= */
...
const DB_VERSION = 2;
/* V2: 'internshipSites' neu. Reihenfolge egal – Laden ist key-basiert.
   Versions-Bump legt fehlende Stores an, bestehende Daten bleiben erhalten. */
const STORES = ['subjects', 'tasks', 'exams', 'grades', 'appointments', 'internshipSites', 'internship', 'images'];
let _db = null;
...
const state = {
  view: 'dashboard',
  internView: null, // { siteId } → Detailansicht einer Praktikumsstelle, sonst Liste
...
async function loadAll() {
  const results = await Promise.all(STORES.map((s) => dbAll(s)));
...
function siteName(id) { const s = state.internshipSites.find((x) => x.id === id); return s ? s.name : 'Praktikum'; }
function daysForSite(id) { return state.internship.filter((n) => n.siteId === id); }
function imagesFor(type, id) { return state.images.filter((im) => im.relatedItemType === type && im.relatedItemId === id); }

/* V2-Datennetzwerk: Klausur ↔ Note.
   Die Note mit examId ist die einzige Quelle für das Klausurergebnis –
   dadurch fließt ein Klausurergebnis automatisch in Fach- und Gesamtschnitt. */
function gradeForExam(examId) { return state.grades.find((g) => g.examId === examId) || null; }
function examResult(examId) { const g = gradeForExam(examId); return g ? g.points : null; }
/* Klausurergebnis schreiben/aktualisieren/entfernen (eine Note pro Klausur). */
async function setExamResult(exam, points) {
  const existing = gradeForExam(exam.id);
  if (points == null) { if (existing) await dbDel('grades', existing.id); return; }
  await dbPut('grades', {
    id: existing ? existing.id : uid(),
    subjectId: exam.subjectId,
    examId: exam.id,
    points,
    label: exam.title || 'Klausur',
    kind: 'exam',
...
function openExamForm(existing) {
  const t = existing || {};
  const currentResult = existing ? examResult(existing.id) : null; // aus verknüpfter Note
  ...
  m.body.innerHTML = `
    ...
    <div class="field">
      <label>Ergebnis <span class="hint">nach der Rückgabe · fließt automatisch in den Schnitt</span>
        <button type="button" class="hint-btn" data-ptsclear>zurücksetzen</button>
      </label>
      ${ptsPicker('exam', currentResult)}
    </div>
  ...
  m.foot.innerHTML = `
    ...
    <button class="btn primary" data-save>Speichern</button>`;
  wirePtsPicker(m.el);
  $('[data-ptsclear]', m.el).onclick = () => {
    $$('.pts-btn', m.el).forEach((x) => x.classList.remove('on'));
    $('[data-ptsval="exam"]', m.el).value = '';
  };

  const img = mountImageEditor($('[data-imghost]', m.el), existing ? imagesFor('exam', existing.id) : []);

  if (existing) $('[data-del]', m.el).onclick = async () => {
    if (await confirmDialog('Diese Klausur, ihr Ergebnis und zugehörige Bilder löschen?')) {
      const linked = gradeForExam(existing.id);
      if (linked) await dbDel('grades', linked.id);
      for (const im of imagesFor('exam', existing.id)) await dbDel('images', im.id);
      await dbDel('exams', existing.id);
      await Promise.all([reload('exams'), reload('grades'), reload('images')]);
  ...
  $('[data-save]', m.el).onclick = async () => {
    const g = (f) => $(`[data-f="${f}"]`, m.el);
    const id = existing ? existing.id : uid();
    const pv = $('[data-ptsval="exam"]', m.el).value;
    const points = pv === '' ? null : parseInt(pv, 10);
    const rec = {
      id,
    ...
    await setExamResult(rec, points);           // Note anlegen/aktualisieren/entfernen
    await img.persist('exam', id, { category: 'Klausur', subjectId: rec.subjectId, date: rec.date, title: rec.title });
    await Promise.all([reload('exams'), reload('grades'), reload('images')]);
    m.close(); render();
    toast(points != null ? 'Klausur & Ergebnis gespeichert' : (existing ? 'Klausur gespeichert' : 'Klausur erstellt'));
  };
}

function openGradeForm(existing, presetSubject) {
  const t = existing || {};
  const fromExam = !!t.examId; // Note gehört zu einer Klausur
  const linkedExam = fromExam ? byId('exams', t.examId) : null;
  const m = openModal({ title: existing ? 'Note bearbeiten' : 'Note hinzufügen' });
  m.body.innerHTML = `
    ${fromExam ? `<div class="link-note">${icon('clipboard')}<span>Gehört zur Klausur „${esc(linkedExam?.title || t.label || 'Klausur')}“. Fach und Ergebnis bearbeitest du am besten direkt in der Klausur.</span></div>` : ''}
    <div class="row2">
      <div class="field"><label>Fach</label><select class="select" data-f="subjectId" ${fromExam ? 'disabled' : ''}>${subjectOptions(t.subjectId || presetSubject, false)}</select></div>
      <div class="field"><label>Datum</label><input class="input" type="date" data-f="date" value="${esc(t.date || todayISO())}"></div>
    </div>
  ...
  if (existing) $('[data-del]', m.el).onclick = async () => {
    const msg = fromExam ? 'Ergebnis dieser Klausur entfernen? Die Klausur selbst bleibt bestehen.' : 'Diese Note löschen?';
    if (await confirmDialog(msg)) {
      await dbDel('grades', existing.id); await reload('grades');
      m.close(); render(); toast(fromExam ? 'Ergebnis entfernt' : 'Note gelöscht');
    }
  };
  $('[data-save]', m.el).onclick = async () => {
    const g = (f) => $(`[data-f="${f}"]`, m.el);
    ...
    const subjectId = fromExam ? t.subjectId : g('subjectId').value;
...
function siteOptions(selected) {
  return state.internshipSites
    .map((s) => `<option value="${s.id}" ${s.id === selected ? 'selected' : ''}>${esc(s.name)}</option>`)
    .join('');
}

function openInternshipForm(existing, presetSiteId) {
  const t = existing || {};
  const siteId = t.siteId || presetSiteId || state.internshipSites[0]?.id || null;
  const m = openModal({ title: existing ? 'Praktikumstag bearbeiten' : 'Neuer Praktikumstag' });
  m.body.innerHTML = `
    <div class="row2">
      <div class="field"><label>Praktikumsstelle</label><select class="select" data-f="siteId">${siteOptions(siteId)}</select></div>
      <div class="field"><label>Datum</label><input class="input" type="date" data-f="date" value="${esc(t.date || todayISO())}"></div>
    </div>
    <div class="row2">
  ...
  $('[data-save]', m.el).onclick = async () => {
    const g = (f) => $(`[data-f="${f}"]`, m.el);
    const id = existing ? existing.id : uid();
    const chosenSite = g('siteId').value || null;
    const rec = {
    ...
    await img.persist('internship', id, { category: 'Praktikum', subjectId: null, date: rec.date, title: siteName(chosenSite) });
    await reload('internship'); await reload('images');
    if (chosenSite) state.internView = { siteId: chosenSite };
    m.close(); render(); toast(existing ? 'Praktikumstag gespeichert' : 'Praktikumstag erstellt');
  };
}

function openInternshipSiteForm(existing) {
  const t = existing || {};
  const m = openModal({ title: existing ? 'Praktikumsstelle bearbeiten' : 'Neue Praktikumsstelle' });
  m.body.innerHTML = `
    <div class="field"><label>Name der Einrichtung</label><input class="input" data-f="name" value="${esc(t.name || '')}" placeholder="z. B. Seniorenheim Sonnenhof"></div>
    <div class="field"><label>Art der Einrichtung <span class="hint">optional</span></label><input class="input" data-f="kind" value="${esc(t.kind || '')}" placeholder="z. B. Altenpflege, Kindergarten, Klinik …"></div>
    <div class="field"><label>Adresse <span class="hint">optional</span></label><input class="input" data-f="address" value="${esc(t.address || '')}" placeholder="Straße, PLZ Ort"></div>
    <div class="row2">
      <div class="field"><label>Ansprechpartner <span class="hint">optional</span></label><input class="input" data-f="contact" value="${esc(t.contact || '')}" placeholder="Name"></div>
      <div class="field"><label>Telefon <span class="hint">optional</span></label><input class="input" type="tel" data-f="phone" value="${esc(t.phone || '')}" placeholder="Telefonnummer"></div>
    </div>
    <div class="field"><label>E-Mail <span class="hint">optional</span></label><input class="input" type="email" data-f="email" value="${esc(t.email || '')}" placeholder="E-Mail"></div>
    <div class="row2">
      <div class="field"><label>Von <span class="hint">optional</span></label><input class="input" type="date" data-f="periodStart" value="${esc(t.periodStart || '')}"></div>
      <div class="field"><label>Bis <span class="hint">optional</span></label><input class="input" type="date" data-f="periodEnd" value="${esc(t.periodEnd || '')}"></div>
    </div>
    <div class="field"><label>Notizen <span class="hint">optional</span></label><textarea class="textarea" data-f="note" placeholder="Weitere Infos zur Stelle">${esc(t.note || '')}</textarea></div>
    <div class="field"><label>Logo / Bild <span class="hint">optional</span></label><div data-imghost></div></div>`;
  m.foot.innerHTML = `
    ${existing ? `<button class="btn danger" data-del>Löschen</button>` : ''}
    <button class="btn ghost" data-close>Abbrechen</button>
    <button class="btn primary" data-save>Speichern</button>`;
  ...
  const img = mountImageEditor($('[data-imghost]', m.el), existing ? imagesFor('internshipSite', existing.id) : []);

  if (existing) $('[data-del]', m.el).onclick = async () => {
    const days = daysForSite(existing.id);
    const dayImgs = days.reduce((n, d) => n + imagesFor('internship', d.id).length, 0);
    const msg = days.length
      ? `„${existing.name}“ mit ${days.length} Praktikumstag${days.length === 1 ? '' : 'en'} und allen zugehörigen Bildern löschen?`
      : `„${existing.name}“ löschen?`;
    if (await confirmDialog(msg)) {
      for (const d of days) {
        for (const im of imagesFor('internship', d.id)) await dbDel('images', im.id);
        await dbDel('internship', d.id);
      }
      for (const im of imagesFor('internshipSite', existing.id)) await dbDel('images', im.id);
      await dbDel('internshipSites', existing.id);
      await Promise.all([reload('internshipSites'), reload('internship'), reload('images')]);
      state.internView = null;
      m.close(); render(); toast(`Stelle gelöscht${dayImgs ? '' : ''}`);
    }
  };
  $('[data-save]', m.el).onclick = async () => {
    const g = (f) => $(`[data-f="${f}"]`, m.el);
    const name = g('name').value.trim();
    if (!name) { g('name').focus(); toast('Name fehlt'); return; }
    const id = existing ? existing.id : uid();
    const rec = {
      id, name,
      kind: g('kind').value.trim(),
      address: g('address').value.trim(),
      contact: g('contact').value.trim(),
      phone: g('phone').value.trim(),
      email: g('email').value.trim(),
      periodStart: g('periodStart').value || null,
      periodEnd: g('periodEnd').value || null,
      note: g('note').value.trim(),
      createdAt: existing?.createdAt || Date.now(),
    };
    await dbPut('internshipSites', rec);
    await img.persist('internshipSite', id, { category: 'Praktikum', subjectId: null, date: todayISO(), title: name });
    await reload('internshipSites'); await reload('images');
    state.internView = { siteId: id };
    m.close(); render(); toast(existing ? 'Stelle gespeichert' : 'Stelle erstellt');
  };
}
...
function renderDashboard() {
  ...
  const todayItems = [
    ...
    ...todayIntern.map((i) => ({ t: 'Praktikum · ' + esc(siteName(i.siteId)), m: i.report ? i.report.slice(0, 40) : 'Tagesbericht', cls: 'accent', act: `openIntern:${i.id}` })),
  ];
  ...
  view.innerHTML = `
    ...
    <div class="grid">
      ...
      <div class="card interactive" data-act="goto:internship">
        ...
        ${lastIntern ? `<div class="mini-t" style="font-size:14.5px">${esc(siteName(lastIntern.siteId))}</div>
          <div class="stat-sub" style="margin-top:4px">${esc(fmtLong.format(parseISO(lastIntern.date)))}${lastIntern.report ? ' · ' + esc(lastIntern.report.slice(0, 44)) : ''}</div>`
...
function renderCalendar() {
  ...
  for (let i = 0; i < 21; i++) {
    ...
    state.internship.filter((n) => n.date === iso).forEach((n) => events.push({ type: 'intern', time: '', title: 'Praktikum · ' + siteName(n.siteId), sub: n.report ? n.report.slice(0, 50) : 'Tagesbericht', act: `openIntern:${n.id}` }));
...
function examRow(e) {
  ...
  const result = examResult(e.id);
  let end;
  if (result != null) end = `<span class="grade-avg-badge" style="font-size:14px">${result} P</span>`;
  else if (past) end = `<span class="tag-muted">nicht bewertet</span>`;
  else end = `<span class="due ${meta.cls}">${esc(relDayName(e.date))}</span>`;
...
/* --- Praktikum (V2: Stelle → Tage) --- */
function renderInternship() {
  // Detailansicht einer Stelle?
  if (state.internView && state.internView.siteId) {
    const site = state.internshipSites.find((s) => s.id === state.internView.siteId);
    if (site) return renderInternshipSite(site);
    state.internView = null; // Stelle existiert nicht mehr → zurück zur Liste
  }
  const sites = [...state.internshipSites].sort((a, b) => a.name.localeCompare(b.name, 'de'));
  $('#view').innerHTML = `
    <div class="page-head">
...
function fmtPeriod(a, b) {
  if (a && b) return `${fmtShort(a)} – ${fmtShort(b)}`;
  if (a) return `ab ${fmtShort(a)}`;
  if (b) return `bis ${fmtShort(b)}`;
  return '';
}
...
function navigate(view) {
  state.view = view;
  state.internView = null; // Nav landet immer auf der Stellen-Liste, nie in einer Detailansicht
...
/* Detailansicht einer Praktikumsstelle öffnen (ohne navigate, damit internView gesetzt bleibt) */
function openSiteDetail(id) {
  if (!byId('internshipSites', id)) return;
  state.view = 'internship';
  state.internView = { siteId: id };
...
function runSearch(q) {
  ...
  const res = {
    ...
    Praktikumsstellen: state.internshipSites.filter((s) => has(s.name) || has(s.kind) || has(s.address) || has(s.contact) || has(s.note)).slice(0, 6)
      .map((s) => ({ icon: 'briefcase', t: s.name, m: [s.kind, `${daysForSite(s.id).length} Tage`].filter(Boolean).join(' · '), act: `openSite:${s.id}` })),
    Praktikumstage: state.internship.filter((n) => has(n.report) || has(n.note) || has(siteName(n.siteId)) || has(relDayName(n.date))).slice(0, 5)
      .map((n) => ({ icon: 'clock', t: `${siteName(n.siteId)} · ${fmtShort(n.date)}`, m: n.report ? n.report.slice(0, 40) : '', act: `openIntern:${n.id}` })),
...
/* V1 → V2 Migration (additiv, zerstörungsfrei):
   1. Praktikumstage aus V1 hatten keine siteId → einer automatisch angelegten
      Sammel-Stelle „Mein Praktikum“ zuordnen, damit sie sichtbar bleiben.
   2. V1-Klausuren mit direkt gespeicherten Punkten (exam.points) → in die
      verknüpfte Note überführen, damit sie in Fach-/Gesamtschnitt zählen.
   Es wird nichts gelöscht und nichts dupliziert. */
async function migrateData() {
  let touchedSites = false, touchedDays = false, touchedGrades = false;

  const orphanDays = state.internship.filter((n) => !n.siteId);
  if (orphanDays.length) {
    let bucket = state.internshipSites.find((s) => s.migratedDefault);
    if (!bucket) {
      bucket = {
        id: uid(), name: 'Mein Praktikum', kind: '', address: '',
        contact: '', phone: '', email: '', periodStart: null, periodEnd: null,
        note: 'Automatisch aus früheren Einträgen angelegt. Du kannst sie umbenennen.',
        migratedDefault: true, createdAt: Date.now(),
      };
      await dbPut('internshipSites', bucket);
      touchedSites = true;
    }
    for (const n of orphanDays) { n.siteId = bucket.id; await dbPut('internship', n); }
    touchedDays = true;
  }

  for (const ex of state.exams) {
    if (ex.points != null && !gradeForExam(ex.id)) {
      await setExamResult(ex, ex.points);
      touchedGrades = true;
    }
  }

  const jobs = [];
  if (touchedSites) jobs.push(reload('internshipSites'));
  if (touchedDays) jobs.push(reload('internship'));
  if (touchedGrades) jobs.push(reload('grades'));
  if (jobs.length) await Promise.all(jobs);
}

/* ---- Lokales Backup: Export / Import (JSON, inkl. Bilder als Data-URL) ---- */
...
async function exportBackup() {
  try {
    ...
    const data = { app: 'fos-dashboard', version: DB_VERSION, exportedAt: new Date().toISOString(), stores: {} };
    for (const s of STORES) {
      const rows = await dbAll(s);
      if (s === 'images') {
        data.stores[s] = await Promise.all(rows.map(async (r) => ({
          ...r,
    ...
    a.href = url;
    a.download = `fos-dashboard-backup-${todayISO()}.json`;
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 4000);
    toast('Sicherung gespeichert');
  } catch (err) { console.error(err); toast('Sicherung fehlgeschlagen'); }
}
function importBackupPrompt() {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      if (!data || !data.stores) { toast('Ungültige Sicherungsdatei'); return; }
      const count = Object.values(data.stores).reduce((n, arr) => n + (Array.isArray(arr) ? arr.length : 0), 0);
      const ok = await confirmDialog(
        `${count} Einträge aus der Sicherung übernehmen? Einträge mit gleicher ID werden aktualisiert – es wird nichts gelöscht.`,
        { danger: false, okLabel: 'Importieren' }
      );
      if (!ok) return;
      toast('Import läuft …');
      for (const s of STORES) {
        ...
        for (const r of rows) {
          if (s === 'images') {
            const rec = { ...r };
            if (typeof rec.blob === 'string') rec.blob = dataURLToBlob(rec.blob);
            if (typeof rec.thumb === 'string') rec.thumb = dataURLToBlob(rec.thumb);
